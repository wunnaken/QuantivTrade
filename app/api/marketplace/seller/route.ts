...
export async function GET() {
  ...
  const [listingsResult, purchasesResult, reviewsResult] = await Promise.allSettled([
    supabase
      ...
      .select(
        "id, title, category, categories, status, view_count, sales_count, avg_rating, review_count, price, price_type, subscription_interval, description, tags, asset_classes, backtest_data, content_data, preview_image_url, preview_disabled, created_at, rejection_reason, discount_percent, discount_enabled, discount_expires_at"
      )
      .eq("seller_id", userId)
